...
$credito_total_general=0;

$reporte->renglon = array ("BANCOS","", "", "", "");
$reporte->imprimir_renglon();

$id_cuenta_caja = $this->config->que_configuracion($this->id_empresa . "_cuenta_caja", "numero");

$this->db->sql  = "select * from cuentas \n";
$this->db->sql .= "where id_cuenta=" . $id_cuenta_caja . " \n";
$this->db->sql .= "and id_empresa=" . $this->id_empresa . " \n";
$this->db->sql .= "limit 1";

$this->db->query_db();

$id_cuenta_disponibilidad = $this->db->campo("id_cuenta_padre");

// echo "cuenta_disponibilidad: " . $id_cuenta_disponibilidad . "<br>";

$this->db->sql  = "select * from cuentas \n";
$this->db->sql .= "where id_cuenta_padre = " . $id_cuenta_disponibilidad . " \n";
$this->db->sql .= "and id_empresa=" . $this->id_empresa . " \n";
$this->db->sql .= "and id_cuenta <> " . $id_cuenta_caja . " \n";


if (!$this->db->query_db()) {
    echo "Error, no se encontraron cuentas disponibilidad!";
    die;
}
...
do {
            ...
            $reporte->renglon = array ($this->db->campo("cuenta"),"", "", "", "");
            $reporte->imprimir_renglon();
            ...
            $db2->sql  = "select * from asientos \n";
            $db2->sql .= "where fecha='" . $this->fecha . "' \n";
            $db2->sql .= "and id_cuenta=" . $id_cuenta . " \n";
            $db2->sql .= "and tipo_asiento='credito' \n";
            $db2->sql .= "and estado='activo' \n";
            $db2->sql .= "order by fecha";

            if ($db2->query_db()) {

                $credito_total=0;
                do {
                    $credito=$aritmetica->formato_numero($db2->campo("importe"),2);
                    $credito_total += $credito;

                    $credito_total_general += $credito;

                    $reporte->renglon = array (
                            $clienteproveedor->que($db2->campo("id_clienteproveedor"),"razon_social"),
                            $db2->campo("fecha"),
                            $db2->campo("detalle"),
                            '',
                            $credito
                  ...
                  $reporte->renglon = array ("","","",
                  "TOTAL $",
                  $aritmetica->formato_numero($credito_total, 2));
                  $reporte->imprimir_renglon();
...
$reporte->renglon = array ("","","",
"TOTAL BANCOS $",
$aritmetica->formato_numero($credito_total_general, 2));
$reporte->imprimir_renglon();
            ...
            $reporte->renglon = array ("GASTOS DE CAJA","", "", "", "");
            $reporte->imprimir_renglon();
            $reporte->renglon = array ("Cliente Proveedor","Fecha", "Detalle", "", "Importe");
            $reporte->imprimir_renglon();
